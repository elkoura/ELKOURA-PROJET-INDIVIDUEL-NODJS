    ...
    try {
      ...
      if (!selectedBiere)
        return res.status(404).json({
          err,
          message: `La biere avec l'id: '${req.params.id_bar}' n'existe pas.`,
        });
      ...
      res
        .status(200)
        .json({ message: "Beer updated successfully", beer: updatedBeer });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  },
  ...
  deleteBiere: async (req, res) => {
    try {
      ...
      return res.status(200).json({
        // , along side with ${ commandeDestroyed } order(s
        message: `${biere.name} has been destroyed) `,
      });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
