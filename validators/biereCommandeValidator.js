  ...
  return (
    ...
    function (req, res, next) {
      ...
      if (req.method === "POST") {
        ...
        Promise.all([
          Bieres.findOne({ where: { id: biere_id } }),
          Commandes.findOne({ where: { id: commande_id } }),
        ]).then(([biere, commande]) => {
          if (!biere || !commande) {
      ...
      if (req.method === "DELETE") {
        ...
        BiereCommandes.findOne({ where: { biere_id, commande_id } })
          .then((biereCommande) => {
            if (!biereCommande) {
              return res.status(404).json({
                error: `La commande avec l'ID ${commande_id} et la bière avec l'ID ${biere_id} n'existe pas.`,
              });
            }
            next();
          })
